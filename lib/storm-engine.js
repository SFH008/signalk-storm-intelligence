...
function geometryForClipping(geometry, referenceLongitude) {
  ...
  return polygons.map(polygon =>
    polygon.map(ring =>
      ring.map(point => [
        unwrapLongitude(Number(point[0]), referenceLongitude),
        Number(point[1])
      ])
    )
  )
...
function polygonIoU(a,b) {
  const ca=centroidGeometry(a)
  if(!ca)return 0

  const aa=geometryForClipping(a,ca[0])
  const bb=geometryForClipping(b,ca[0])

  if(!aa||!bb)return 0

  try {
    const intersection=polygonClipping.intersection(aa,bb)
    ...
    return unionArea>0
      ? Math.max(0,Math.min(1,intersectionArea/unionArea))
      : 0
  } catch {
    return 0
  }
...
function predictedTrackCentroid(track,history,epochMs) {
  ...
  if(
    ...
    return localPoint(last,velocity.east*dt,velocity.north*dt)
  }
...
class StormEngine {
  constructor(config={}) {
    this.config={
      ...
      maxMissedFrames:1,
      pathStepSec:60,
      baseUncertaintyM:1500,
      maxUncertaintyM:20000,
      ...config
    }
  ...
  evaluate(snapshot, vessel) {
    ...
    const current=(snapshot?.features||[])
      .map((f,i)=>({
    ...
    const active=[...this.trackState.entries()].map(([trackId,state])=>({
      trackId,
      ...state,
    ...
    for(const track of active){
      const predicted=predictedTrackCentroid(track,track.history,epochMs)

      for(let currentIndex=0;currentIndex<current.length;currentIndex++){
        const cell=current[currentIndex]
        const iou=polygonIoU(track.geometry,cell.geometry)
        const distanceMeters=predicted
          ? haversine(predicted,cell.centroid)
          : Infinity

        if(!(iou>0 || distanceMeters<=this.config.matchDistanceM))continue

        const distanceScore=Math.max(
          0,
          Math.min(
            1,
            distanceMeters/Math.max(1,this.config.matchDistanceM)
          )
        )
    ...
    for(const [currentIndex,survivor] of assignmentByCurrent){
      ...
      for(const competitor of competitors){
        const state=this.trackState.get(competitor.trackId)
        if(!state || mergedTracks.has(competitor.trackId))continue
    ...
    const vv=vesselVelocity(vessel)
    const vp=vessel?.position
      ? [vessel.position.longitude,vessel.position.latitude]
      : null

    const seenTracks=new Set()

    const results=current.map((cell,currentIndex)=>{
      const assigned=assignmentByCurrent.get(currentIndex)
      ...
      if(!trackId){
        ...
        parentTrackId=splitParent?.trackId||null
      ...
      const history=(
        previousObservation?.epochMs===epochMs
          ? [...old.slice(0,-1),{epochMs,centroid:cell.centroid}]
          : [...old,{epochMs,centroid:cell.centroid}]
      ).slice(-Math.max(2,this.config.historyFrames))
      ...
      this.trackState.set(trackId,{
        ...
        firstSeenMs:existing?.firstSeenMs??epochMs,
        lastSeenMs:epochMs,
        observations:(existing?.observations||0)+1,
        missedFrames:0,
        parentTrackId:existing?.parentTrackId??parentTrackId??null
      })

      const vel=robustTrackVelocity(history)
      const distance=vp?distanceToGeometryMeters(vp,cell.geometry):null
      const approach=vp
        ? cpa(vp,cell.centroid,vel,vv,this.config.horizonMinutes*60)
        : null

      const uncertainty=Math.min(
        this.config.maxUncertaintyM,
        this.config.baseUncertaintyM+
        (1-vel.confidence)*12000+
        (vel.residualMeters||0)
      )
      ...
      const pathThreat=vp
        ? pathPolygonThreat(
            ...
            cell.geometry,
            vel,
            this.config.horizonMinutes*60,
            {
              stepSec:this.config.pathStepSec,
      ...
      if(approach?.closing)relevantDistance=Math.min(
        relevantDistance,
        approach.dcpaMeters
      )
      if(pathThreat)relevantDistance=Math.min(
        relevantDistance,
        pathThreat.minDistanceMeters
      )

      const state=relevantDistance==null
        ? 'normal'
        : severityState(severity,relevantDistance,this.config)

      const threat={
        state,
        confidence:vel.confidence,
        method:'polygon-path',
        intersects:pathThreat?.intersects||false,
        interceptSec:pathThreat?.interceptSec??null,
        minDistanceMeters:pathThreat?.minDistanceMeters??null,
        minDistanceSec:pathThreat?.minDistanceSec??null,
      ...
      return {
        ...
        trackId,
        sourceId:cell.sourceId,
        parentTrackId:parentTrackId??existing?.parentTrackId??null,
        geometry:cell.geometry,
        ...
        history:history.map(h=>({
          time:new Date(h.epochMs).toISOString(),
          centroid:h.centroid
        }))
      }
    })

    for(const [trackId,state] of [...this.trackState.entries()]){
      if(seenTracks.has(trackId) || mergedTracks.has(trackId))continue

      const missedFrames=(state.missedFrames||0)+1

      if(missedFrames>this.config.maxMissedFrames){
        this.closedTracks.push({
          trackId,
          lifecycle:{
            status:'expired',
    ...
    results.sort((a,b)=>
      ...
      (
        (a.threat?.minDistanceMeters??Infinity) -
        (b.threat?.minDistanceMeters??Infinity)
      )
    )
