...
const { StormTracker } = require('./storm-tracker')
...
class StormEngine {
  constructor(config={}) {
    ...
    this.tracker=new StormTracker(
      {
        matchDistanceM:this.config.matchDistanceM,
        historyFrames:this.config.historyFrames,
        maxMissedFrames:this.config.maxMissedFrames
      },
      {
        centroidGeometry,
        polygonIoU,
  ...
  snapshotState() {
    return this.tracker.snapshotState()
  }

  evaluate(snapshot, vessel) {
    const epochMs=Number(snapshot?.epochMs)

    const current=this.tracker.update(
      snapshot?.features||[],
      epochMs
    )
    ...
    const results=current.map(cell=>{
      ...
      return {
        ...
        parentTrackId:cell.parentTrackId,
