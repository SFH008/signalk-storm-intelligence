...
class IpmaProvider {
  ...
  async downloadRaw(product, epochMs) {
    ...
    if (!Number.isFinite(requested)) {
      throw new TypeError('IPMA raw acquisition requires observation epochMs')
    }
    ...
    if (!frame) {
      throw new Error(`IPMA ${product} frame not found for ${epochMs}`)
    }

    const imageBase =
      (this.cfg.imageBase || IMAGE_BASE).replace(/\/?$/, '/')
    ...
    if (!contentType.toLowerCase().includes('image/png')) {
      throw new Error(
        `Unexpected IPMA radar content type ${contentType}`
      )
    }
