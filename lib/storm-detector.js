...
function detectReflectivityObjects(field,options={}) {
  ...
  if(!method) {
    throw new TypeError('Detection method is required')
  }

  /*
   * Normalized Storm Evidence v1:
   * downstream science consumes one provider-neutral numerical field and
   * explicit semantics. Legacy reflectivity field names remain accepted
   * temporarily so existing replay/tests can migrate incrementally.
   */
  const normalizedValues=field.values
  const semantics=field.valueSemantics

  const legacyLowerBoundValues=
    field.reflectivityLowerBoundDbz

  const legacyEstimateValues=
    field.reflectivityDbz

  const values=
    normalizedValues ??
    legacyLowerBoundValues ??
    legacyEstimateValues

  if(!values) {
    throw new TypeError('Reflectivity values are required')
  }

  /*
   * New Normalized Storm Evidence v1 inputs use the canonical semantic
   * vocabulary: measurement | lower-bound | estimate.
   *
   * Legacy callers may still carry the older representation string in
   * provenance (for example derived-reflectivity-estimate). Preserve that
   * output during migration so existing observable behaviour does not
   * change merely because the detector learned the new input contract.
   */
  const representation=
    semantics?.representation ??
    field.provenance?.representation ??
    (
      legacyLowerBoundValues!==undefined &&
      legacyLowerBoundValues!==null
        ? 'reflectivity-lower-bound'
        : 'reflectivity-estimate'
    )

  /*
   * Lower-bound behaviour must be determined from the scientific input
   * semantics, not by matching legacy representation label strings.
   */
  const isLowerBound=
    semantics
      ? semantics.representation==='lower-bound'
      : (
          legacyLowerBoundValues!==undefined &&
          legacyLowerBoundValues!==null
        )
  ...
  for(const component of components) {
    let areaKm2=0
    let maxReflectivityDbz=-Infinity
...
function detectRadarEchoStructures(field,options={}) {
  return detectReflectivityObjects(field,{
    ...options,
    minReflectivityDbz:options.minReflectivityDbz ?? 12,
    method:'reflectivity-echo-structure-development-v1'
  })
}
...
function detectStormCandidates(field,options={}) {
  return detectReflectivityObjects(field,{
    ...options,
    minReflectivityDbz:options.minReflectivityDbz ?? 24,
